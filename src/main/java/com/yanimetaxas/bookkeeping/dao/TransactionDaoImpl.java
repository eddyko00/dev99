...
/**
 * The Data Object Access support to abstract and encapsulate all access to the
 * transaction_history and transaction_leg tables of the data source.
 *
 * @author yanimetaxas
 * @since 14-Nov-14
 */
public class TransactionDaoImpl extends JdbcDaoSupport implements TransactionDao {
    ...
    public void storeTransaction(Transaction transaction) {
        StringBuilder sql = new StringBuilder()
                ...
                .append("(client_ref, transaction_ref, transaction_type, transaction_date, updatedatel, comment)")
                .append(" VALUES (?, ?, ?, ?, ?, ?)");
    ...
    public List<Transaction> getTransactions(List<String> transactionRefs) {
        StringBuilder sql = new StringBuilder()
                .append("SELECT transaction_ref, transaction_type, transaction_date, updatedatel, comment ")
    ...
    private class TransactionMapper implements RowMapper<Transaction> {
        ...
        public Transaction mapRow(@Nonnull ResultSet rs, int rowNum) throws SQLException {
            ...
            long updatedatel = rs.getLong("updatedatel");
            String comment = rs.getString("comment");
            List<TransactionLeg> legs = getLegsForTransaction(transactionRef);
            return new Transaction(transactionRef, transactionType, transactionDate, updatedatel, comment, legs);
        }
    }
